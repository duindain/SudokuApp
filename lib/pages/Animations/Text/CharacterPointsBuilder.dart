...
class CharacterPointsBuilder {
  ...
  List<Offset>? generatePointsForCharacter(String character) {
    ...
    if (_pathCache[character] != null)
      return _pathCache[character];
  ...
  List<Offset> _centerAlignPoints(List<Offset> points) {
    ...
    for (var point in points) {
      if (top == null || top < point.dy)
        top = point.dy;
      if (bottom == null || bottom > point.dy)
        bottom = point.dy;
      if (left == null || left < point.dx)
        left = point.dx;
      if (right == null || right > point.dx)
        right = point.dx;
    }
